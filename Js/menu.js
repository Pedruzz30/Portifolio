...
export function setupMenu({ menuToggle, menuSpans, navOverlay, navLinks, getCssVar }) {
  ...
  const toggleMenu = () => {
    ...
    const openColor = getCssVar('--accent') || safeGetComputedStyle('--accent') || '#ff4d4d';
    const closedColor = getCssVar('--text') || safeGetComputedStyle('--text') || '#f5f5f5';
  ...
  const closeMenu = () => {
    ...
    if (spans.length && window.gsap) {
      const closedColor = getCssVar('--text') || safeGetComputedStyle('--text') || '#f5f5f5';
      gsap.to(spans, { backgroundColor: closedColor, y: 0, rotate: 0, duration: 0.3 });
    }
  };
